...
enum ProxyProtocolCatalog {
    ...
    static let coreTypeIDs = [
        ...
        "openvpn", "tailscale", "shadowquic", "zerotier", "easytier",
    ]
    ...
    static let walledTypeIDs: [String] = {
#if os(macOS)
        []
#else
        ["easytier"]
    ...
    static let formalTypeIDs = coreTypeIDs.filter { !walledTypeIDs.contains($0) }

    static let specifications: [ProxyProtocolSpecification] =
        allSpecifications.filter { !walledTypeIDs.contains($0.typeID) }

    private static let allSpecifications: [ProxyProtocolSpecification] = [
        ...
        .init(typeID: "easytier", displayName: "EasyTier", summary: "Embedded EasyTier mesh outbound that joins a network by name and secret and routes through its peers.", aliases: ["easy tier"]),
    ]

    private static let lookup: [String: ProxyProtocolSpecification] = {
        var result: [String: ProxyProtocolSpecification] = [:]
        for item in allSpecifications {
    ...
    static let simulatedConfigurations: [[String: Any]] = [
        ...
        ["name": "EasyTier Demo", "type": "easytier", "network-name": "office-mesh", "network-secret": "configured-secret", "hostname": "hako-demo", "ipv4": "10.144.144.2/24", "dhcp": false, "peers": ["tcp://peer.example:11010"], "listeners": ["tcp://0.0.0.0:11010"], "instance-name": "hako", "state-dir": "Configured", "udp": true, "accept-dns": true, "enable-encryption": true, "encryption-algorithm": "aes-gcm", "latency-first": false, "mtu": 1380, "local-private-key": "configured-secret", "local-public-key": "configured-public"],
    ]
    ...
    private static func protocolRows(
        ...
        case "easytier":
            ...
            appendPrivateString(&rows, "Network", "network-name", mapping)
            appendConfigured(&rows, "Network Secret", keys: ["network-secret"], mapping)
            appendPrivateString(&rows, "Hostname", "hostname", mapping)
            appendPrivateString(&rows, "Virtual IPv4", "ipv4", mapping)
            appendBool(&rows, "DHCP", "dhcp", mapping)
            appendCount(&rows, "Peers", "peers", mapping)
            appendCount(&rows, "Listeners", "listeners", mapping)
            appendConfigured(&rows, "State Storage", keys: ["state-dir"], mapping)
            appendBool(&rows, "UDP", "udp", mapping)
            appendBool(&rows, "Encryption", "enable-encryption", mapping)
            appendString(&rows, "Encryption Algorithm", "encryption-algorithm", mapping)
            appendNumber(&rows, "MTU", "mtu", mapping)
            appendConfigured(&rows, "Key Pair", keys: ["local-private-key", "local-public-key"], mapping)
        default:
            break
        }
        return rows
    ...
    private static func sectionTitle(for typeID: String) -> String {
        ...
        case "wireguard", "masque", "openvpn", "tailscale", "zerotier", "easytier":
            return "Tunnel"
...
struct ProxyEditorSchemaField: Decodable, Identifiable {
    ...
    private static let secretKeys: Set<String> = [
        "auth", "auth-key", "identity-secret", "key", "local-private-key",
        "network-secret", "obfs-password", "password", "pre-shared-key",
...
enum ProxyProtocolEditorSchema {
    ...
    static let fieldsByType: [String: [ProxyEditorSchemaField]] = {
        ...
        assert(result.count == 28, "Bundled proxy protocol schema must cover every parser type")
        assert(
            Set(ProxyProtocolCatalog.coreTypeIDs) == Set(result.keys),
            """
            ...
            create. Missing here: \
            \(Set(result.keys).subtracting(ProxyProtocolCatalog.coreTypeIDs).sorted())
            """
        )
